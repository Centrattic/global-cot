...
def process_responses():
    ...
    for json_file in json_files:
        ...
        print(
            f"Processed {json_file}: index {response_data.get('index', 0)} -> processed_index {processed_index - 1}"
        )
    ...
    correct_count = sum(1 for entry in processed_responses
                        if entry["correctness"])
    print(
        f"Correct responses: {correct_count}/{len(processed_responses)} ({correct_count/len(processed_responses)*100:.1f}%)"
    )
