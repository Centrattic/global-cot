class Edge:
    ...
    def print(self):
        print(
            f"Edge: {self.from_cluster} -> {self.to_cluster} (freq: {self.freq})"
        )
...
class Node:
    ...
    def __init__(self,
        ...
        self.mean_cosine_similarity = self._compute_mean_cosine_similarity(
            sentence_embeddings)

    def _compute_mean_cosine_similarity(
            self, sentence_embeddings: Optional[List[List[float]]]) -> float:
...
class AnswerNode(Node):
    ...
    def __init__(self, true_answer: int, candidate_answers: List[str]):
        super().__init(f"Answer: {true_answer}", 0,
                       candidate_answers)  # freq is meaningless here
