...
os.environ["TIKTOKEN_ENCODINGS_BASE"] = os.path.expanduser(
    "~/.cache/oh/encodings")

from openai_harmony import (load_harmony_encoding, HarmonyEncodingName, Role,
                            Message, Conversation, DeveloperContent,
...
class ActivationExtractor:
    ...
    def __init__(self,
                 ...
                 cache_dir: Path = Path("activation_cache")):
    ...
    def extract_activations(self,
        ...
        tokens = self.enc.render_conversation_for_completion(
        ...
        print(
            f"[DEBUG] Raw completion text: {self.tokenizer.decode(completion_tokens)}"
        )
        ...
        if response_start in completion_text and response_end in completion_text:
            start_idx = completion_text.find(response_start) + len(
                response_start)
        ...
        # Store activations
        self._store_activations(layer_activations, cot_content,
                                response_content, cot_tokens, seed)
    ...
    def _create_streamer(self):
        ...
        return ConsoleStreamer(self.tokenizer,
                               skip_prompt=True,
                               skip_special_tokens=True)

    def _extract_layer_activations(self, tokens: List[int],
        ...
        try:
            ...
            def hook_fn(module, input, output):
                ...
                if isinstance(output, tuple):
                    activations = output[0].detach().cpu().to(
                        torch.float32).numpy()
                else:
                    activations = output.detach().cpu().to(
                        torch.float32).numpy()
    ...
    def _store_activations(self, layer_activations: Dict[int,
                                                         Dict[str,
                                                              np.ndarray]],
                           cot_content: str, response_content: str,
                           cot_tokens: List[int], seed: int):
        ...
        for layer, activations in layer_activations.items():
            ...
            sentence_activations = self._aggregate_by_sentences(
                activations, sentences, cot_content, cot_tokens)
    ...
    def _aggregate_by_sentences(
            self, activations: np.ndarray, sentences: List[str],
            cot_content: str, cot_tokens: List[int]) -> Dict[str, np.ndarray]:
        ...
        for sentence in sentences:
            ...
            sentence_activations_tokens = activations[
                current_index:current_index + sentence_length]
    ...
    def load_activations(
            self, layer: int,
            completion_index: int) -> Optional[Dict[str, np.ndarray]]:
    ...
    def get_completion_info(self,
                            completion_index: int) -> Optional[Dict[str, Any]]:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Extract activations from GPT-OSS-20B model')
    parser.add_argument('--start',
                        type=int,
                        default=0,
                        help='Start index for completion range (default: 0)')
    parser.add_argument('--end',
                        type=int,
                        default=1,
                        help='End index for completion range (default: 1)')
    ...
    import os
    # set visible GPU to GPU we want to use
    ...
    print(
        f"Processing completions {idx_start} to {idx_end-1} (total: {num_completions})"
    )
